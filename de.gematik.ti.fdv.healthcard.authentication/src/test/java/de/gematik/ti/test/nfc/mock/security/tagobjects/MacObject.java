...
package de.gematik.ti.test.nfc.mock.security.tagobjects;
...
public class MacObject {
    ...
    private void calculateMac() {
        ...
        final CMac cbcMac = getCMac(ssc, kMac);

        synchronized (cbcMac) {
            if (header != null) {
                final byte[] paddedHeader = Bytes.padData(header, BLOCK_SIZE);
                cbcMac.update(paddedHeader, 0, paddedHeader.length);
            }

            if (dataOutput.size() > 0) {
                final byte[] paddedData = Bytes.padData(dataOutput.toByteArray(), BLOCK_SIZE);
    ...
    public static CMac getCMac(final byte[] secureMessagingSSC, final byte[] kMac) {
        final CMac cmac = new CMac(new AESEngine());
